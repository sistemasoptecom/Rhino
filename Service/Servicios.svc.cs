...
namespace Rhino.Service
{
    ...
    public class Servicios : IServicios
    {
        ...
        public string CreateProveedores(string json)
        {
            dynamic data = jss.DeserializeObject(json);
            string _json = data["json"];
            try
            {
                ...
                proveedor prov = jss.Deserialize<proveedor>(_json);
                conn.proveedor.Add(prov);
                int rs = conn.SaveChanges();
                return jss.Serialize(rs > 0);
            }
        ...
        public string UpdateProveedores(string json)
        {
            dynamic data = jss.DeserializeObject(json);
            string _json = data["json"];
            int rs = 0;
            proveedor prov = jss.Deserialize<proveedor>(_json);
            conn.Entry(prov).State = System.Data.EntityState.Modified;
            rs = conn.SaveChanges();
            return jss.Serialize(rs > 0);
        }
        public string ReadProveedores(string json)
        {
            dynamic data = jss.DeserializeObject(json);
            int id = data["id"];
            proveedor c = conn.proveedor.Where(a => a.id.Equals(id)).FirstOrDefault();
            return jss.Serialize(c);
        }
        public string DeleteProveedores(string json)
        {
            dynamic data = jss.DeserializeObject(json);
            int id = data["id"];
            proveedor proveedor = conn.proveedor.Where(a => a.id.Equals(id)).FirstOrDefault();
            conn.proveedor.Remove(proveedor);
            int rs = conn.SaveChanges();
            return jss.Serialize(rs);
        }
        public string ValidateProveedores(string json)
        {
            //bool b = false;
            //dynamic data = jss.DeserializeObject(json);
            //string nit = data["nit"];
            //bool u = false;
            //u = conn.proveedor.Where(z => z.nit.Equals(nit)).Count() > 0;
            //return jss.Serialize(u);

            dynamic data = jss.DeserializeObject(json);
            string nit = Convert.ToString(data["nit"]);
            proveedor p = conn.proveedor.Where(a => a.nit.Equals(nit)).FirstOrDefault();
        ...
        public string PreloadProveedores()
        {
            List<object> arr = new List<object>();
            List<proveedor> prov = conn.proveedor.ToList();
            arr.Add(prov);
            return jss.Serialize(arr);
        }
